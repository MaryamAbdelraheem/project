...
function Books() {
  ...
  const filterByTypeAndCategory = (products) => {
    return products.filter(item => item.type === 'book');
  };
        ...
        <div className={`border border-gray-300 pl-5 py-3 my-5 ${showFilter ? '' : 'hidden'} sm:block`}>
          ...
          <div className='flex flex-col gap-2 text-sm font-light text-gray-700'>
            <p className='flex gap-2'>
              <input className='w-3' type="checkbox" value={'cs'} onChange={toggleSubCategory} /> Computer Science
            </p>
            <p className='flex gap-2'>
              <input className='w-3' type="checkbox" value={'ai'} onChange={toggleSubCategory} /> Artificail Intelligence
            </p>
          </div>
        </div>
      </div>
